...
from app.crud import mine_transport as crud
from app.models.user import User
from app.schemas.mine_transport import (
    MineTransportCreate,
    MineTransportListResponse,
    MineTransportPaymentUpdate,
    MineTransportResponse,
    MineTransportUpdate,
)
from app.utils.excel_import import import_excel
from app.utils.jalali import jalali_to_gregorian

router = APIRouter(prefix="/mine-transport", tags=["mine-transport"])
...
@router.get("", response_model=MineTransportListResponse)
def list_transports(
    ...
    return MineTransportListResponse(items=items, total=total, page=page, size=size)


@router.get("/{transport_id}", response_model=MineTransportResponse)
def read_transport(
...
@router.post("", response_model=MineTransportResponse, status_code=status.HTTP_201_CREATED)
def create_transport(
    obj_in: MineTransportCreate,
...
@router.put("/{transport_id}", response_model=MineTransportResponse)
def update_transport(
    transport_id: uuid.UUID,
    obj_in: MineTransportUpdate,
    _: Annotated[User, Depends(get_current_user)],
    db: Annotated[Session, Depends(get_db)],
) -> MineTransportResponse:
    obj = crud.get(db, transport_id)
...
@router.post("/{transport_id}/pay", response_model=MineTransportResponse)
def mark_paid(
    transport_id: uuid.UUID,
    payment: MineTransportPaymentUpdate,
    _: Annotated[User, Depends(get_current_user)],
    db: Annotated[Session, Depends(get_db)],
) -> MineTransportResponse:
    obj = crud.get(db, transport_id)
...
@router.post("/{transport_id}/upload-bol", response_model=MineTransportResponse)
...
@router.post("/{transport_id}/upload-payment-receipt", response_model=MineTransportResponse)
...
async def import_excel_endpoint(
    ...
    for i, row in enumerate(rows, start=1):
        try:
            ...
            destination = crud_site.get_by_code(db, dest_code) if dest_code else None
            ...
            if not driver:
                ...
                continue
            if not destination:
                row_errors.append(f"Row {i}: destination '{dest_code}' not found")
                continue
